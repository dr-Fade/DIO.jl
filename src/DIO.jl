...
# keep the config and preallocated arrays in an object for easier handling and better performance
struct Dio
    f0_ceil::Float32
    f0_floor::Float32
    sample_rate::Integer
    ...
    Dio(f0_ceil, f0_floor, sample_rate; relative_peak_threshold=0.25, noise_floor=0.01) = begin
        ...
        DSP.periodogram
        low_cutoff = f0_floor / 2
        bands = 0:Int(round(log2(2 * f0_ceil / f0_floor)))
        high_cutoffs = [f0_floor * 2^b for b in bands]
        filters = [
            begin
                filter_type = DSP.Bandpass(low_cutoff, high_cutoff)
                h = digitalfilter(filter_type, window; fs=sample_rate) |> Vector{Float32}
                FIRFilter(h, DOWNSAMPLED_RATE // sample_rate)
            end
            for high_cutoff ∈ high_cutoffs
        ]
        new(
            f0_ceil,
            f0_floor,
            ...
            filters,
            [zeros(Float32, decimated_frame_length) for i ∈ 1:length(high_cutoffs)],
            DOWNSAMPLED_RATE / 3 / f0_ceil |> round |> Int,
            DOWNSAMPLED_RATE / 2 / f0_floor |> round |> Int,
...
detect_pitch(dio::Dio) = sample::Vector{Float32} -> begin
    ...
    # @info "FROM DETECT PITCH" detected_points
...
(dio::Dio)(sample::Vector{Float32}; previous_estimate::F0Estimate=(value=0f0, confidence=0f0)) =
    sample |>
...
dio_contour(dio::Dio, sound::Vector{Float64}; hop::Float32=1f0)::Matrix{Float32} = dio_contour(dio, sound .|> Float32; hop=hop)

# return an estimated f0 curve and the confidence
dio_contour(dio::Dio, sound::Vector{Float32}; hop::Float32=1f0)::Matrix{Float32} = begin
    N = length(sound)
    n = hop * dio.sample_rate / 1000f0 |> round |> Int
    frames = reshape(sound[1:(end-N%n)], n, N ÷ n)
    res = Matrix{Float32}(undef, 2, size(frames)[end])
    for (i, frame) ∈ enumerate(eachcol(frames))
        previous_estimate = if i > 1
            (value=res[1, i-1], confidence=res[2, i-1])
        else
            (value=0f0, confidence=0f0)
        end
        estimate = dio(Vector(frame); previous_estimate=previous_estimate)
        res[1, i] = estimate.value
        res[2, i] = estimate.confidence
    end
    return res
end
